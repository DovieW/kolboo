...
This document explains how the **Usage → Spend** system works end-to-end (backend + frontend), and how to extend it with new **providers/models**, new **cost estimators**, and new **UI features**.
...
- Sidebar page: **Usage**
- **Activity** (default): words, recordings, audio duration, active days and UTC trends.
- **Models**: recording/word/audio totals per transcription model.
- **Spend**: estimated API costs and filters; hidden for signed-in Personal/Pro
  and Enterprise/Business accounts.
- One timeframe selector applies to all tabs.

Activity is a separate, content-free summary of retained History, generated by
`history/activity.rs` through `get_history_activity`. It counts successful
recordings once by recording request ID (entry ID for legacy records), using the
earliest retained successful result and its original text. Manual edits do not
change transcribed-word counts. Missing audio durations are not estimated. The
frontend fetches only numeric aggregates/model IDs and invalidates on
`history-changed`; it never loads the full transcript archive for statistics.
These local totals shrink when history is deleted and are not billing records.
...
- `app/src/components/usageStats/UsageStatsView.tsx`
  - `UsageStatsView()`
  - renders Activity, Models and the account-appropriate Spend tab

### Cost tab component

- `app/src/components/usageStats/CostTab.tsx`
  - estimated-spend card and provider totals
