...
- Pipeline safety patterns:
  ...
  - Frontend Logs View presentational sections live under `app/src/components/logs/**`; keep `LogsView.tsx` focused on query hooks, export dialogs, playback wiring, hotkey-debug toggles, and other adapter-side orchestration.
...
- Toggle/Hold/Paste Last shortcut action dispatch lives in `app/src-tauri/src/shortcuts/{toggle_recording,hold_recording,paste_last}.rs`, while registration remains in `shortcuts/lifecycle.rs`, Windows hook mechanics remain in `windows_modifier_hotkeys.rs`, and Quick Ask / Escape cancel routing stays in `shortcuts/mod.rs`.
...
- Frontend Data Lifecycle normalization lives in `app/src/lib/settings/dataLifecycle.ts`, Data Settings presentational sections live in `app/src/components/settings/data/**`, History Feed read-model logic lives in `app/src/lib/history/readModel.ts`, Logs View read-model logic lives in `app/src/lib/logs/readModel.ts`, Logs View presentational sections live in `app/src/components/logs/**`, frontend query hooks are domain-shaped under `app/src/lib/queries/**`, and query-function factories are domain-shaped under `app/src/lib/queries/queryFns/**`.
