...
export function QuickAskPanel({
	...
	>;
	setQuickAskActiveWindowOcrModeInheriting: Dispatch<SetStateAction<boolean>>;
	...
	setLocalProfileQuickAskIncludeClipboardContext: Dispatch<
		SetStateAction<boolean>
	>;
	setLocalProfileQuickAskActiveWindowOcrMode: Dispatch<
	...
	return (
		<>
			...
			<div
				className={[
					"settings-row",
					ocrModeDisabled ? "settings-row--disabled" : null,
				]
					.filter(Boolean)
					.join(" ")}
			>
				<div>
					<p className="settings-label">Active Window OCR</p>
					<p className="settings-description">
						Optionally capture the currently active window, run OCR, and include
						the text as extra context during Quick Ask.
					</p>
				</div>
				<div style={{ display: "flex", alignItems: "center", gap: 10 }}>
					<SettingsInheritanceIndicator
						isDefaultScope={isDefaultScope}
						inheriting={quickAskActiveWindowOcrModeInheriting}
						inheritTooltip={inheritTooltip}
						onDisableOverride={() =>
							openDisableOverrideDialog({
								title: "Disable Quick Ask Active Window OCR override?",
								onConfirm: () => {
									setQuickAskActiveWindowOcrModeInheriting(true);
									saveProfileMetadata({
						...
						disabled={isSavingProfile || ocrModeDisabled}
					/>
					<Tooltip
					...
					>
						<div>
							<Select
								...
								value={localProfileQuickAskActiveWindowOcrMode}
								onChange={(value) => {
									if (!value) return;
									if (
										value === "off" ||
										...
										setLocalProfileQuickAskActiveWindowOcrMode(value);
										saveProfileMetadata({
											quick_ask_active_window_ocr_mode: value,
										});
									}
								}}
								disabled={isSavingProfile || ocrModeDisabled}
