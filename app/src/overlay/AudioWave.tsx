			...
			unlisten = await listenTyped("overlay-audio-level", (p) => {
				...
				if (
					...
					for (let i = 0; i < n; i++) {
						...
						maxes[i] = Number.isFinite(mx) ? Math.max(-1, Math.min(1, mx)) : 0;
					}
					...
					if (waveHistBinsRef.current !== n || !waveHistBufRef.current) {
						...
						waveHistWriteRef.current = 0;
						waveHistBufRef.current = new Float32Array(WAVE_HISTORY_FRAMES * n);
...
export { AudioWave, BackendAudioWave, type AudioWaveProps };
