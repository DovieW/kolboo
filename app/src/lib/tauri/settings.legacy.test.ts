...
describe("legacy settings fixtures", () => {
	...
	itWithImportTimeout(
		"normalizes malformed legacy defaults without overwriting explicit nulls",
		async () => {
			vi.resetModules();
			currentStore = new FakeStore({
				settings_version: 2,
				...
				output_mode: "keystrokes_and_clipboard",
				...
				rewrite_program_prompt_profiles: [
					{
						id: "legacy",
						name: "Legacy",
						program_path: "C:\\Legacy\\legacy.exe",
						cleanup_prompt_sections: { main: "old" },
						stt_language: "bad-profile-language",
			...
			expect(settings.settings_version).toBe(2);
			expect(settings.stt_language).toBe("en");
			expect(settings.overlay_monitor_target).toBe("active_window");
			expect(settings.output_mode).toBe("paste");
			expect(settings.quick_ask_system_prompt).toBeNull();
			expect(settings.quick_ask_hold_hotkey).toBeNull();

			const profile = settings.rewrite_program_prompt_profiles[0];
			expect(profile?.program_paths).toEqual(["C:\\Legacy\\legacy.exe"]);
			expect(profile?.stt_language).toBeNull();
			expect(profile?.overlay_mode).toBeNull();
		},
	);
});
