...
impl ElevenLabsSttProvider {
    ...
    pub fn new(api_key: String, model: Option<String>, language: Option<String>) -> Self {
        ...
        Self {
            client,
            api_key,
            // Default to Scribe v2 so the provider can take the realtime path by default.
    ...
    fn should_use_realtime_api(&self) -> bool {
        ...
        // We map the user-facing `scribe_v2` choice to `scribe_v2_realtime` inside the
        // provider-local realtime module rather than teaching the rest of the app that name.
        let model = self.model.trim().to_lowercase();
        model == "scribe_v2" || model == "scribe_v2_realtime"
    }
...
impl SttProvider for ElevenLabsSttProvider {
    async fn transcribe(&self, audio: &[u8], format: &AudioFormat) -> Result<String, SttError> {
        if self.should_use_realtime_api() {
            return realtime::transcribe_realtime_ws(self, audio, format).await;
        }
...
mod tests {
    ...
    #[test]
    fn test_supports_streaming_for_realtime_model_family() {
        let realtime = ElevenLabsSttProvider::new("test-key".to_string(), None, None);
        assert!(realtime.supports_streaming());

        let legacy =
            ElevenLabsSttProvider::new("test-key".to_string(), Some("scribe_v1".to_string()), None);
        assert!(!legacy.supports_streaming());
    }
}
