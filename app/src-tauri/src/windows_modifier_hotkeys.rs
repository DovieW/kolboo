...
use windows::Win32::UI::WindowsAndMessaging::{
    ...
    KBDLLHOOKSTRUCT, MSG, PM_REMOVE, WH_KEYBOARD_LL, WM_KEYDOWN, WM_KEYUP, WM_QUIT, WM_SYSKEYDOWN,
    WM_SYSKEYUP,
};
...
fn emit_hotkey_debug_event(app: &AppHandle, message: &str, details: Option<String>) {
    // Clone into owned values so we can move into async task.
    // Emit asynchronously to avoid blocking the low-level hook callback.
    // Blocking in a WH_KEYBOARD_LL hook can cause Windows to stop calling it.
    let app = app.clone();
...
pub fn init(app: AppHandle) {
    ...
    std::thread::Builder::new()
        ...
        .spawn(|| unsafe {
            ...
            // Use PeekMessageW with a sleep to avoid blocking indefinitely,
            // which allows the hook to remain responsive.
            let mut msg = MSG::default();
            loop {
                // Process all pending messages
                while PeekMessageW(&mut msg, None, 0, 0, PM_REMOVE).into() {
                    if msg.message == WM_QUIT {
                        log::info!("Modifier-only hotkey hook thread exiting (WM_QUIT)");
                        return;
                    }
...
unsafe extern "system" fn low_level_keyboard_proc(
    ...
    if is_down || is_up {
        ...
        //
        // Skip injected events: when Kolboo (WebView2/Chromium) is focused, synthetic key events
        // may be generated by the browser for accessibility or internal handling. These should
        // not trigger typing suppression or the shortcut won't fire when the app is focused.
        let is_injected = (flags & LLKHF_INJECTED) != 0;
        if is_down && ALT_RIGHT_HELD.load(Ordering::Relaxed) && !is_modifier_vk(vk) && !is_injected
        {
