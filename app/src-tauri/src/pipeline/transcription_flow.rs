...
impl TranscriptionContext<'_> {
    /// Check ownership while holding the request-log lock, not before taking it.
    /// Cancellation can release capture before an old provider response arrives.
    fn update_request_log(&self, update: impl FnOnce(&mut crate::request_log::RequestLog)) {
...
async fn run_llm_router<C: TranscriptionCallbacks>(
    ...
    let llm_out = tokio::select! {
        biased;
        _ = ctx.cancel_token.cancelled() => return None,
        result = route_preset_id_with_llm(profile, stt_text, provider.as_ref()) => result,
    };
...
async fn run_embeddings_router<C: TranscriptionCallbacks>(
    ...
    let router_start = std::time::Instant::now();
    let embeddings_out = tokio::select! {
        biased;
        _ = ctx.cancel_token.cancelled() => return None,
        result = route_preset_id_with_embeddings(
        profile,
        stt_text,
        &ctx.proxy_settings,
        &ctx.llm_api_keys,
...
fn log_router_scores(
    ...
    if ctx.request_log_store.is_some() {
...
fn log_embeddings_router_scores(
    ...
    if ctx.request_log_store.is_none() {
        return;
    }
...
#[cfg(test)]
#[path = "transcription_flow/tests.rs"]
mod tests;
