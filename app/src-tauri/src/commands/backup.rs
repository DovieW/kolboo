...
fn load_settings_json_from_disk(app: &AppHandle) -> Result<serde_json::Value, String> {
	...
	let bytes = std::fs::read(&settings_path)
		.map_err(|e| format!("Failed to read settings file {}: {}", settings_path.display(), e))?;
	let v: serde_json::Value = serde_json::from_slice(&bytes)
		.map_err(|e| format!("Failed to parse settings file {}: {}", settings_path.display(), e))?;
	Ok(v)
}
...
#[cfg(desktop)]
#[tauri::command]
pub async fn github_backup_push_to_gist(app: AppHandle, gist_id: Option<String>) -> Result<String, String> {
	...
	let body = if gist_id.as_deref().filter(|s| !s.trim().is_empty()).is_some() {
		serde_json::json!({
			"files": {
	...
	let req = if gist_id.as_deref().filter(|s| !s.trim().is_empty()).is_some() {
		client.patch(url)
	} else {
		client.post(url)
	};
...
#[cfg(not(desktop))]
#[tauri::command]
pub async fn github_backup_push_to_gist(_app: AppHandle, _gist_id: Option<String>) -> Result<String, String> {
	Ok("".to_string())
}

#[cfg(desktop)]
#[tauri::command]
pub async fn github_backup_pull_from_gist(app: AppHandle, gist_id: String) -> Result<String, String> {
	let token = github_token(&app)?;
	let id = gist_id.trim();
	if id.is_empty() {
	...
	let v: serde_json::Value = resp.json().await.map_err(|e| e.to_string())?;
	let files = v.get("files").and_then(|f| f.as_object()).cloned().unwrap_or_default();
...
#[cfg(not(desktop))]
#[tauri::command]
pub async fn github_backup_pull_from_gist(_app: AppHandle, _gist_id: String) -> Result<String, String> {
	Ok("".to_string())
}
