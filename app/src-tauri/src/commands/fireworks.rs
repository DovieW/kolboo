...
fn persist_fireworks_models_disk_cache(
	app: &AppHandle,
	api_key_fp: u64,
	models: &[ModelOption],
) {
	let Ok(path) = fireworks_models_cache_path(app) else { return; };
...
fn normalize_label(model: &FireworksModel) -> String {
	if let Some(d) = model.display_name.as_deref().map(|s| s.trim()).filter(|s| !s.is_empty()) {
		return d.to_string();
	}
...
pub async fn fireworks_list_models(app: AppHandle) -> Result<Vec<ModelOption>, String> {
	...
	for _ in 0..10 {
		...
		if page_token.as_deref().map(|s| s.trim().is_empty()).unwrap_or(true) {
			break;
		}
	}
	...
	for m in catalog_models.into_iter() {
		...
		display_by_id.entry(m.name.clone()).or_insert_with(|| normalize_label(&m));
	}
